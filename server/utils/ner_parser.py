...
class NerResumeParser:
    ...
    def _parse_resume_structured(self, text):
        ...
        resume_data = {}
        try:
            # ner_results = self.predictions.ner(text) 
            # print(ner_results)
            doc = self.predictions.get_spacy_doc(text)
    ...
    def _run_zero_shot_classification_on_results(self, tokens, labels, desiredLabels):
        element = ""
        confidence = 0
        for token in tokens:
            res = self.predictions.zero_shot_classifier(token, labels)
            print("res: ", res)
            ...
            if highest[1] > confidence:
                if (highest[0] in desiredLabels):
                    element = token.strip()
                    confidence = highest[1]
        return element

    def _extract_job_title(self, tokens, labels, desiredLabels):
        new_title = []
        for token in tokens:
    ...
    def _extract_entity_sort_based(self, candidate_companies, labels):
        scores = []
        for comp in candidate_companies:
            res = self.predictions.zero_shot_classifier(comp, labels)['scores']
    ...
    def _extract_entities_from_job_header(self, header_array):
        ...
        print("header_array: ", header_array)
        ...
        # remove the date from the parsed header
        header_line = header_line.replace(result["date_range"], "")
        print("header_line with date removed: ", header_line)

        # clean and split by any punctuation marks
        job_title_splitter = re.compile(r'[{}]+'.format(re.escape(punctuation.replace("&", "") )))
        job_title = ''.join(i for i in header_line if not i.isdigit())
        tokens = job_title_splitter.split(header_line)
        tokens = [''.join([i for i in tok.strip() if (i.isalpha() or i.strip()=="")]) for tok in tokens if tok.strip()]
        print("tokens: ", tokens)
        cleaned_header_line = " ".join(tokens)

        ner_results = self.predictions.ner(header_line) 
        # found_entities = {result["entity_group"] for result in ner_results}
        ...
        print("ner_results: ", ner_results)
        for ent in doc.ents:
            print("label: ", ent.label_, "word: ", ent.text)
        for ner_result in ner_results:
            ...
            entity_group = ner_result["entity_group"]
            word = capitalize_clean_text(ner_result["word"])
            if entity_group == "Designation":
                result["job_title"].append(word)
        ...
        print("result before zero shot: ", result) 
        # # run zero shot classification to distinguish company and location,
        # # skip if company classification if self employed 

        # # if work mode and self employed is found, extract
        # if result["work_mode"] != "":
        #     header_line = header_line.replace(result["work_mode"], "")

        # if self_employed:
        #     header_line = header_line.replace(self_employed, "")

        # tokens = job_title_splitter.split(header_line)
        # tokens = [''.join([i for i in tok.strip() if (i.isalpha() or i.strip()=="")]) for tok in tokens if tok.strip()]

        # result["job_title"] = self._extract_job_title(
        #     tokens + result["job_title"],
        #     ["company", "organization", "institution", "job title", "responsibility", "details"], 
        #     ["job title"]
        # )

        # print("job_title: ", result["job_title"])

        # if type(result["job_title"]) == str:
        #     header_line = header_line.replace(result["job_title"], "")

        # print("header_line after replacements: ", header_line)

        # tokens = job_title_splitter.split(header_line)
        # tokens = [''.join([i for i in tok.strip() if (i.isalpha() or i.strip()=="")]) for tok in tokens if tok.strip()]

        # result["company"] = self._extract_entity_sort_based(
        #     tokens + result["company"], 
        #     ["company", "organization", "institution", "not institution", "not organization", "not company"],

        # ) if self_employed == "" else self_employed 

        # result["location"] = self._extract_entity_sort_based(
        #     tokens + result["location"], 
        #     ["location", "city", "institution", "organization", "not institution", "not organization"],
        # )
